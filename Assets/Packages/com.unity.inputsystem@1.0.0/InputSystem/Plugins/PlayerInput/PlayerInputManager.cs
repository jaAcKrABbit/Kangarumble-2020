    ...
    public class PlayerInputManager : MonoBehaviour
    {
        ...
        public PlayerInput JoinPlayer(int playerIndex = -1, int splitScreenIndex = -1, string controlScheme = null, InputDevice pairWithDevice = null)
        {
            if (!CheckIfPlayerCanJoin(playerIndex))
                return null;
            if (!CheckIfDeviceOverlap (controlScheme, pairWithDevice))
                return null;
        ...
        private bool CheckIfDeviceOverlap(string scheme, InputDevice device)
        {
            ...
            if (device.name == "Keyboard")
            {
                for (var i = 0; i < activePlayers.Count(); i++)
                    if (activePlayers[i].GetComponent<PlayerInput>().currentControlScheme == scheme)
                    {
                        Debug.Log(
                            $"Keyboard scheme {scheme} is already taken by player {PlayerInput.s_AllActivePlayers[i+1]}");
                        return false;
                    }
            }
            else
            {
                for (var i = 0; i < activePlayers.Count(); i++)
                    if (activePlayers[i].GetComponent<PlayerInput>().devices[0] == device)
                    {
                        Debug.Log(
                            $"Gamepad {device} is already taken by player {PlayerInput.s_AllActivePlayers[i+1]}");
                        return false;
                    }
            }
            return true;
